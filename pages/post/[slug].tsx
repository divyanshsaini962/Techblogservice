...
// import post from '../../tech-blog-service/schemas/post'
   ...
   return (
      ...
      <main className="font-bodyFont ">
          <Header />
           ...
           {/* main-image */}
           <img className="w-full h-96 object-cover" src={urlFor(post.mainImage).url()!} alt="cover-image" />
             ...
             <div className="max-w-3xl mx-auto mb-10">
                <article className="w-full mx-auto p-5 bg-secondaryColor/10">
                   ...
                   <div className='flex items-center gap-3'>
                    <img className="rounded-full w-12 h-12 object-cover bg-red-400" src={urlFor(post.author.image).url()} alt="author-img" />
                      <p className='font-bodyFont text-base'>
                        Blog post by{" "} 
                        <span className='font-bold text-seondaryColor'>{post.author.name}</span>{" "} 
                        - Pulished at {new Date(post.publishedAt).toLocaleDateString()}</p>
                   </div>
                   <div className='mt-10'>
                     <PortableText
                       ...
                       serializers={{
                         ...
                         li:({children}:any)=>(
                            <li
                              className='ml04 list-disc'>{children}</li>   
                         ),
                ...
                <hr className='max-w-lg my-5 mx-auto boder[1px] border-secondaryColor' />
                {
                  submitted ? <div className="flex flex-col items-center gap-2 p-10 my-10 bg-bgColor text-white mx-auto">
                    <h1 className="text-2xl font-bold">Thenk you for submitting your Comment</h1>
                    <p>Once it has been approved, it will uppear below!</p>
                  </div> : <div>
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className='mt-7 flex flex-col'>
                    ...
                    {
                      session && (
                    <button className='w-full bg-bgColor text-white text-base font-titleFont font-semibold 
                    tracking-wider uppercase py-2 rounded-sm hover:bg-secondaryColor duration-300'
                  ...
                  {
                      !session && (
                    <button onClick={handleUserErr} className='w-full bg-bgColor text-white text-base font-titleFont font-semibold 
                    tracking-wider uppercase py-2 rounded-sm hover:bg-secondaryColor duration-300'
                     type='submit'>Submit</button>
                      )
                    }
                   ...
                   userErr && (
                     <p className="text-sm font-titleFont text-center font-semibold text-red-500 underline underline-offset-2 my-1 px-4 animate-bounce">{" "} 
                       <span className="text-base font-bold italic mr-2">!</span>{userErr}</p>
                   )}
                  {/* Comments */}
